...
from src.common.constants import ParserTag, Position, Stroke
...
class TestDictToScoreConverter(unittest.TestCase):
    ...
    def test_create_rest_stave(self):
        rest_stave = self.converter._create_rest_stave(Position.PEMADE_POLOS, Stroke.SILENCE, 2.5)
        self.assertEqual(len(rest_stave), 3)
        self.assertEqual(rest_stave[0].total_duration, 1)
        self.assertEqual(rest_stave[1].total_duration, 1)
        self.assertEqual(rest_stave[2].total_duration, 0.5)
