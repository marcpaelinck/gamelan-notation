...
class PositionNote:
    ...
    def note(self, **kwargs) -> Note:
        ...
        defaults = {"octave": 1, "effect": Stroke.OPEN, "note_value": 1}
        all_kwargs = defaults | kwargs
        new_note = NoteFactory.create_note(position=self.position, **all_kwargs)
        transformation = RuleValue.SAME_TONE if new_note.is_melodic() else None
        return NoteFactory.clone_note(new_note, autogenerated=self.autogenerated, transformation=transformation)
    ...
    @property
    def BYOT(self):
        return self.note(pitch=Pitch.BYONG, octave=None, effect=Stroke.ABBREVIATED)

    @property
    def MUTEDSTRIKE(self):
        return self.note(pitch=Pitch.STRIKE, octave=None, effect=Stroke.MUTED)

    @property
    def SILENCE(self):
        return self.note(pitch=Pitch.SILENCE, octave=None, effect=Stroke.NONE, note_value=1.0)

    @property
    def EXTENSION(self):
        return self.note(pitch=Pitch.EXTENSION, octave=None, effect=Stroke.NONE)
