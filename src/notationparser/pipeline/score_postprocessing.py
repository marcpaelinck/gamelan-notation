...
class ScorePostprocessAgent(Agent):
    ...
    def _create_missing_measures(
        ...
        if missing_positions := (
            ...
            # Add two evenly separated kempli beats if KempliMeta indicates so
            if Position.KEMPLI in measures.keys():
                kemplimeta_list = self.score.gongans[beat.gongan_seq].metadata.get(MetaType.KEMPLI, [])
                kemplimeta = {meta.status: meta for meta in kemplimeta_list}
                line = max(p.line or -1 for m in beat.measures.values() for p in m.passes.values())
                # Create an empty measure + pass dict
                pass_dict = {}
                if meta := kemplimeta.get(MetaDataSwitch.DOUBLE, None):
                    passes = meta.passes or [DEFAULT]
                    rests1 = self._create_multiple_rests(
                        Position.KEMPLI, resttype=Pitch.EXTENSION, duration=(beat.max_duration - 2) // 2
                    )
                    rests2 = self._create_multiple_rests(
                        Position.KEMPLI, resttype=Pitch.EXTENSION, duration=(beat.max_duration - 2) - len(rests1)
                    )
                    notes = [self._kempli_beat()] + rests1 + [self._kempli_beat()] + rests2
                    pass_dict |= {p: Measure.Pass(seq=p, line=line, notes=notes, autogenerated=True) for p in passes}
                if meta := kemplimeta.get(MetaDataSwitch.OFF, None):
                    passes = meta.passes or [DEFAULT]
                    notes = self._create_multiple_rests(Position.KEMPLI, Pitch.EXTENSION, beat.max_duration)
                    pass_dict |= {p: Measure.Pass(seq=p, line=line, notes=notes, autogenerated=True) for p in passes}
                if DEFAULT not in pass_dict:
                    if beat.has_kempli_beat:
                        ...
                        notes = [self._kempli_beat()] + rests
                    else:
                        notes = self._create_multiple_rests(
                            Position.KEMPLI, resttype=Pitch.EXTENSION, duration=beat.max_duration
                        )
                    pass_dict |= {DEFAULT: Measure.Pass(seq=DEFAULT, line=line, notes=notes, autogenerated=True)}
                kempli_measure = Measure.new(position=Position.KEMPLI, notes=[], pass_seq=None, autogenerated=True)
                kempli_measure.passes = pass_dict
                measures[Position.KEMPLI] = kempli_measure
    ...
    def _move_beat_to_start(self) -> None:
        ...
        if self._has_kempli_beat(last_gongan):
            ...
            for position in last_beat.measures.keys():
                ...
                new_beat.measures[position] = Measure.new(
                    position=position, notes=[], pass_seq=DEFAULT, autogenerated=False
                )
            ...
            for position, measure in beat.prev.measures.items():
                if notes := measure.passes[DEFAULT].notes:  # only consider default pass.
                    ...
                    if not position in beat.measures:
                        # autogenerated=False because the content originates from the source.
                        beat.measures[position] = Measure.new(
                            position=position, notes=[], pass_seq=DEFAULT, autogenerated=False
                        )
                ...
                case KempliMeta():
                    ...
                    for beat in gongan.beats:
                        if beat.id in meta.beats or not meta.beats:
                            beat.kempli_status = meta.status
                            beat.has_kempli_beat = not (meta.status is MetaDataSwitch.OFF)
