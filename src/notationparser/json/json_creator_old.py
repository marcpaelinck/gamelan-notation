...
from src.notationparser.json.compact_encoder import Compact, CompactEncoder
...
class JSection(BaseModel):
    ...
    def serialize_tempo(self, tempo: list[int, int]):
        return Compact(tempo)

    @field_serializer("staves", when_used="always")
    def serialize_data(self, data: dict[str, JStave]):
        return {pos: Compact(stave.model_dump(exclude_defaults=True)) for pos, stave in data.items()}
