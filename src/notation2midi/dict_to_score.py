...
class ScoreCreatorAgent(Agent):
    ...
    def _move_beat_to_start(self) -> None:
        ...
        if self._has_kempli_beat(last_gongan):
            ...
            new_beat = Beat(
                ...
                prev=last_beat,
                flow=Flow(
                    bpm_start={DEFAULT: last_beat.flow.bpm_end[-1]},
                    bpm_end={DEFAULT: last_beat.flow.bpm_end[-1]},
                    # velocity_start and velocity_end are dict[pass, dict[Position, Velocity]]
                    velocities_start={DEFAULT: last_beat.flow.velocities_end[DEFAULT].copy()},
                    velocities_end={DEFAULT: last_beat.flow.velocities_end[DEFAULT].copy()},
                ),
            )
        ...
        while beat.prev:
            for position, measure in beat.prev.measures.items():
                if notes := measure.passes[DEFAULT].notes:  # only consider default pass.
                    ...
                    beat.flow.get_pass(position, DEFAULT).notes[0:0] = notes_to_move  # insert at beginning
                    ...
                    beat.flow.get_pass(position, DEFAULT).autogenerated = False
    ...
    def _create_rest_measures(
        ...
        prevstrokes = {
            ...
            pos: (prev_beat.flow.get_notes(pos, DEFAULT)[-1].stroke if prev_beat else silence)
    ...
    def process_goto(
        ...
        frombeat.flow.goto[pass_nr] = tobeat  # TODO GOTO REMOVE
        goto_item = Flow.GoTo(
            beat=tobeat,
            passes=[pass_nr],
            frequency=frequency,
        )
        frombeat.flow.goto_[pass_nr] = goto_item
    ...
    def _apply_metadata(self, gongan: Gongan) -> None:
        ...
        for meta in sorted(metadata, key=lambda x: x.processingorder):
            ...
            match meta:
                ...
                case PartMeta():
                    pass
                case RepeatMeta():
                    gongan.beats[-1].flow.repeat = Flow.Repeat(goto=gongan.beats[0], iterations=meta.count)
                case SequenceMeta():
                ...
                case TempoMeta() | DynamicsMeta():
                    changetype = Flow.Change.Type.TEMPO if isinstance(meta, TempoMeta) else Flow.Change.Type.DYNAMICS
                    if (
                    ...
                    if meta.beat_count == 0:
                        ...
                        beat.flow.changes[changetype].update(
                            {
                                pass_seq: Flow.Change(
                                    new_value=meta.value,
                                    positions=meta.positions if changetype == Flow.Change.Type.DYNAMICS else [],
                                    incremental=False,
                                )
                                for pass_seq in meta.passes
                            }
                ...
                case WaitMeta():
                    ...
                    newbeat = Beat(
                        ...
                        flow=Flow(
                            bpm_start={-1: 60},
                            bpm_end=lastbeat.flow.bpm_end,
                            velocities_start=lastbeat.flow.velocities_start.copy(),
                            velocities_end=lastbeat.flow.velocities_end.copy(),
                        ),
                    )
                    if lastbeat.next:
                        ...
                        for rep, beat in lastbeat.flow.goto.items():
                            if rep in meta.passes:
                                newbeat.flow.goto[rep] = beat
                                del lastbeat.flow.goto[rep]
                        lastbeat.flow.goto = {}
                        for rep, gotometa in lastbeat.flow.goto_.items():
                            if rep in meta.passes:
                                newbeat.flow.goto_[rep] = gotometa
                                del lastbeat.flow.goto_[rep]
                        lastbeat.flow.goto_ = {}
        ...
        if haslabel:
            ...
            for beat in gongan.beats:
                if not beat.flow.changes and beat.prev:
                    beat.flow.changes.update(beat.prev.flow.changes)
            ...
            for pass_ in measure.passes.values():
                if not pass_.notes:
                    resttype = (
                        ...
                        or prevbeat.flow.get_notes(position, DEFAULT)[-1].stroke is Stroke.SILENCE
            ...
            for self.curr_beat_id, measures in gongan_info[ParserTag.BEATS].items():
                ...
                new_beat = Beat(
                    ...
                    flow=Flow(
                        bpm_start={
                            DEFAULT: (
                                bpm := self.score.gongans[-1].beats[-1].flow.bpm_end[-1] if self.score.gongans else 0
                            )
                        },
                        bpm_end={DEFAULT: bpm},
                        velocities_start={
                            DEFAULT: (
                                # velocity_start and velocity_end are dict[pass, dict[Position, Velocity]]
                                vel := (
                                    self.score.gongans[-1].beats[-1].flow.velocities_end[-1].copy()
