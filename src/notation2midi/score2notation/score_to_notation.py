...
def aggregate_positions(gongan: Gongan) -> dict[Position, list[str]]:
    """Returns a dict that groups similar positions

    Args:
        gongan (Gongan): gongan to convert

    Returns:
        dict[Position, list[str]]: _description_
    """

    # pos_tags maps positions to short tag values. It contains only the non-silent positions that occur in the gongan.
    # KEMPLI is omitted.
    pos_tags = {
        position: position.shortcode
        for position in sorted(list(gongan.beats[0].measures.keys()), key=lambda x: x.sequence)
        if position != Position.KEMPLI
    }

    # Remove empty staves
    for position in list(pos_tags.keys()):
        if is_silent(gongan, position):
            del pos_tags[position]
    ...
    def try_to_aggregate(positions: list[Position], aggregate_tag: str):
        ...
        if not all(pos in pos_tags.keys() for pos in positions):
            return False
...
def gongan_to_records(gongan: Gongan) -> list[dict[Position | int, list[str]]]:
    ...
    result = (
        ...
        + [
            ...
            if any(position in beat.measures for beat in gongan.beats) and not (is_silent(gongan, position))
        ]
