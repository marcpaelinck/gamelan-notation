...
def clean_staves(gongan: Gongan) -> dict[tuple[Position, PassID], list[list[Note]]]:
    ...
    Removes staves that are completely empty after removing autogenerated notes.
    Args:
...
def stringWidth_fromNotes(measure: list[Note], font_name: str, font_size: int) -> int:
    ...
    return stringWidth("a" * char_count, font_name, font_size)
...
def aggregate_positions(gongan: Gongan) -> dict[tuple[Position, PassID], list[str]]:
    ...
    def try_to_aggregate(positions: list[Position], passid: PassID, aggregate_tag: str):
