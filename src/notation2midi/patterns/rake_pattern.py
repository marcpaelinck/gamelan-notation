        ...
        for note in notes:
            if isinstance(note, Pattern) and note.effect in (PatternType.RAKE_LEFT, PatternType.RAKE_RIGHT):
                ...
                attributes |= {
                    ...
                    Note.Fields.NOTE_VALUE: rakesettings.duration_in_basenotes / abs(end_index - start_index),
                ...
                if rakesettings.duration_in_basenotes < 1:
                    whole_rest = NoteFactory.get_whole_rest_note(note.position, Pitch.EXTENSION)
                    rest = NoteFactory.clone_note(
                        whole_rest, note_value=1 - rakesettings.duration_in_basenotes, autogenerated=True
                    )
                    note.pattern.append(rest)
