...
from src.common.notes import Note, NoteFactory, NoteSymbol
...
class ScorePostprocessAgent(Agent):
    ...
    def _kempli_beat(cls) -> Note:
        ...
        note = Note(
            ...
            effect=Stroke.MUTED,
            note_value=1.0,
            generic_note=NoteSymbol(pitch=Pitch.STRIKE, note_value=1, symbol="x"),
            autogenerated=True,
    ...
    def _extend_measure(self, position: Position, notes: list[Note], duration: float):
        ...
        if measure_duration < duration:
            try:
                notes.append(
                    filler.model_copy(update={"note_value": duration - measure_duration, "autogenerated": True})
                )
            except ValidationError:
                self.logerror(
                    "Could not add rest to beat %s-%s of %s", self.curr_gongan_id, self.curr_measure_id, position
    ...
    def _create_rest_notes(self, position: Position, resttype: Pitch, duration: float) -> list[Note]:
    ...
    def _move_beat_to_start(self) -> None:
        ...
        for position, measure in self.score.gongans[0].beats[0].measures.items():
            measure.passes[DEFAULT].notes.insert(0, NoteFactory.get_whole_rest_note(position, Pitch.SILENCE))
                        ...
                        if beat.id in meta.beats or not meta.beats:
                            for position in meta.positions:
                                ...
                                for pass_seq in meta.passes:
                                    ...
                                    notes = self._create_rest_notes(position, Pitch.SILENCE, beat.duration)
