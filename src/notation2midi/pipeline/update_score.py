...
from src.common.notes import GenericNote, Note, NoteFactory
...
class ScoreUpdateAgent(Agent):
    ...
    def _kempli_beat(cls) -> Note:
        ...
        note = Note(
            ...
            stroke=Stroke.MUTED,
            note_value=1.0,
            generic_note=GenericNote(pitch=Pitch.STRIKE, symbol="x"),
            autogenerated=True,
        ).model_copy(update={"autogenerated": True})
        return note

    # def _recalculate_beat_durations(self) -> None:
    #     for gongan in self.gongan_iterator(self.score):
    #         for beat in self.beat_iterator(gongan):
    #             beat.duration = max(
    #                 sum(note.pattern_duration for note in measure.passes[DEFAULT].notes)
    ...
    def _create_rest_notes(self, position: Position, resttype: Stroke, duration: float) -> list[Note]:
        ...
        notes = []
        whole_rest: Note = NoteFactory.get_whole_rest_note(position, resttype)
        ...
        for position, measure in beat.measures.items():
            for pass_ in measure.passes.values():
                if not pass_.notes:
                    ...
                    pass_.notes = self._create_rest_notes(
        ...
        if missing_positions := (
            ...
            if Position.KEMPLI in measures.keys():  # and has_kempli_beat(gongan):
                if beat.has_kempli_beat:
                    rests = self._create_rest_notes(Position.KEMPLI, Stroke.EXTENSION, beat.max_duration - 1)
                    ...
                    measures[Position.KEMPLI] = self._create_rest_measure(
                        Position.KEMPLI, Stroke.EXTENSION, beat.max_duration
                    )
    ...
    def _move_beat_to_start(self) -> None:
        ...
        while beat.prev:
            for position, measure in beat.prev.measures.items():
                if notes := measure.passes[DEFAULT].notes:  # only consider default pass.
                    ...
                    notes_to_move = []
                    while notes and sum((note.pattern_duration for note in notes_to_move), 0) < 1:
                        notes_to_move.insert(0, notes.pop())
                    # Move orphaned grace note
                    if notes and notes[-1].stroke is Stroke.GRACE_NOTE:
                        notes_to_move.insert(0, notes.pop())
        ...
        for position, measure in self.score.gongans[0].beats[0].measures.items():
            measure.passes[DEFAULT].notes.insert(0, NoteFactory.get_whole_rest_note(position, Stroke.SILENCE))
                                    ...
                                    for idx, note in enumerate(notes):
                                        if note.octave is not None:
                                            oct_note = Note(
                                                ...
                                                pitch=note.pitch,
                                                octave=note.octave + meta.octaves,
                                                stroke=note.stroke,
                                                note_value=note.note_value,
                                                autogenerated=note.autogenerated,
                                                generic_note=note.generic_note,
                                            )
                                            if oct_note:
                        ...
                        if beat.id in meta.beats or not meta.beats:
                            for position in meta.positions:
                                ...
                                for pass_seq in meta.passes:
                                    ...
                                    notes = self._create_rest_notes(position, Stroke.SILENCE, beat.duration)
    ...
    # def _add_pattern_to_rests(self):
    #     for gongan in self.gongan_iterator(self.score):
    #         for beat in self.beat_iterator(gongan):
    #             for measure in beat.measures.values():
    #                 for pass_ in measure.passes.values():
    #                     for note in pass_.notes:
    #                         if note.stroke in [Stroke.SILENCE, Stroke.EXTENSION] and not note.pattern:
    #                             note.pattern.append(note.to_pattern_note())
