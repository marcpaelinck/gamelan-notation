...
def clean_staves(gongan: Gongan) -> dict[tuple[Position, PassID], list[list[Note]]]:
    """Converts a gongan into a dict. Removes autogenerated passes (e.g. created to emulate WAIT metadata).
    Also removes staves that are completely empty after removing autogenerated passes.
    Args:
    ...
    skip_beats = set(
        ...
        if all(pass_.autogenerated for measure in beat.measures.values() for pass_ in measure.passes.values())
    )
    staves = {
        (position, passid): (
            [
                [
                    note
                    for note in beat.measures[position].passes[passid].notes
                    if isinstance(note, Pattern) or not note.autogenerated
                ]
                for beat in gongan.beats
                if beat.id not in skip_beats
                and position in beat.measures
                and passid in beat.measures[position].passes
                and not beat.measures[position].passes[passid].autogenerated
            ]
        )
...
def equivalent(note1: Note, note2: Note, positions: list[Position], metadata: MetaData) -> bool:
    """Determines if two notes correspond with each other by applying instrument transformation rules
